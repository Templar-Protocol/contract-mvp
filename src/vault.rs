...
pub struct Vault {
    ...
    pub loans: Vec<AccountId>, // TODO: use UnorderedSet - using Vec for now for compatibility with NEAR SDK 5.3 serializer
...
impl Vault {
    pub fn new(
        ...
        let id = construct_id(&collateral_asset, &stablecoin);
        (
            id,
            Self {
                collateral_asset,
...
pub fn construct_id(collateral_asset: &AccountIdRef, stablecoin: &AccountIdRef) -> String {
    format!("{collateral_asset}:{stablecoin}")
}
